"""Metropolitan Museum of Art collection-search collector."""
...
SOURCE = "met"
API_NAME = "Met Collection Search"
...
def collect(cfg: Config, month: MonthRange, logger=None) -> List[Dict[str, Any]]:
    ...
    for country_code in cfg.country_codes():
        for domain_code in src_cfg.get("applicable_domains", []):
            ...
            per_q: Dict[str, Any] = {}
            for q in queries:
                data = http_get_json(ENDPOINT, params={"q": q, "hasImages": "false"}, timeout=30, retries=3, logger=logger)
                if data is None:
                    errors += 1
                    per_q[q] = None
                    continue
                tot = int(data.get("total", 0) or 0)
                per_q[q] = tot
                total += tot
    ...
    save_json(raw_blob, raw_dir(SOURCE) / f"met_{month.file_label}.json")
    return out
